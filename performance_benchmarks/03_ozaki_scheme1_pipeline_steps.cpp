...
int main() {
    ...
    // For Scheme I, 8 slices extracts 56 bits (covering the full 53-bit FP64 mantissa)
    std::vector<int> test_slices = {3, 4, 5, 6, 7, 8};
    const int WARMUP = 2;
    const int ITERS = 5;
    ...
    std::cout << "Size,Slices,Step1_Stats(%),Step2_Slice(%),Step3_GEMM(%),Step4_Recon(%),Total(ms)\n";
    ...
    for (size_t N : sizes) {
        ...
        for (int slices : test_slices) {
            // Use WorkspaceScheme1 for Scheme I
            ozablas::WorkspaceScheme1 ws(exec, N, N, N, slices);

            // Warmup (No timing struct passed)
            for (int i = 0; i < WARMUP; i++) {
                ozablas::ozaki_scheme1_gemm(ws, d_A, d_B, d_C);
            }
            exec->synchronize();
            ...
            for (int i = 0; i < ITERS; i++) {
                ...
                ozablas::ozaki_scheme1_gemm(ws, d_A, d_B, d_C, &iter_timings);
